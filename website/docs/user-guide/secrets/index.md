...
You can enable more than one secret source at the same time — for example a team 1Password vault alongside a personal vault plugin. Sources compose per env var with a deterministic precedence ladder:

1. **Your `.env` / shell wins by default.** A source only replaces a pre-existing value when its own `override_existing: true` is set (mapped sources default to true so central rotation works).
...
`override_existing` never lets one source overwrite a var another source already claimed, and no source can ever overwrite another source's bootstrap token (e.g. `OP_SERVICE_ACCOUNT_TOKEN`).

```yaml
secrets:
  sources: [onepassword]     # optional explicit ordering
  onepassword:
    enabled: true
    env:
...
Every credential injected by a source is labelled with its origin — setup flows and `hermes model` show `(from 1Password)` next to detected keys so you always know where a value came from.
...
The bundled set is deliberately closed (same policy as memory providers): 1Password ships in-tree. Everything else — Infisical, Proton Pass, HashiCorp Vault, AWS Secrets Manager, OS keystores — belongs in plugin repos; share them in the Nous Research Discord (`#plugins-skills-and-skins`).

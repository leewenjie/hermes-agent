...
CHAT_SIDEBAR_BOOTSTRAP = (
    Path(__file__).resolve().parent.parent / "web/src/lib/chat-sidebar-bootstrap.ts"
)
...
def test_sidecar_session_create_requests_close_on_disconnect():
    ...
    source = CHAT_SIDEBAR_BOOTSTRAP.read_text(encoding="utf-8")
...
def test_sidecar_session_create_scopes_profile():
    ...
    source = CHAT_SIDEBAR_BOOTSTRAP.read_text(encoding="utf-8")
    assert re.search(r'"session\.create",\s*\{', source), (
        "sidecar session.create call not found"
    )
    assert re.search(r"close_on_disconnect:\s*true", source)
    assert re.search(r'source:\s*"tool"', source)
    assert re.search(r"\.\.\.\(profile\s*\?\s*\{\s*profile\s*\}\s*:\s*\{\}\)", source)

...
def test_profile_local_mcp_tool_is_visible_in_slash_worker(tmp_path):
    ...
    try:
        ...
        proc.stdin.flush()
        try:
            # The worker imports and starts a real MCP server before answering.
            # Parallel CI runners can take longer than ten seconds to complete
            # that startup even though the integration is healthy.

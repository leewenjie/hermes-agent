...
#
# `uv sync --frozen --no-install-project --extra all --extra messaging --extra wecom`
# installs the deps reachable through the composite `[all]` extra
# (handpicked set intended for the production image — excludes `[dev]`),
# plus gateway messaging adapters and the secure XML parser required by the
# WeCom callback adapter. These must work in the published image without a
...
RUN touch ./README.md
RUN uv sync --frozen --no-install-project --extra all --extra messaging --extra wecom --extra anthropic --extra bedrock --extra azure-identity --extra hindsight --extra matrix
